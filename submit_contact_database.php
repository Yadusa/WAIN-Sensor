...
// Save to database
try {
    $stmt = $conn->prepare("INSERT INTO contact_submissions (timestamp, name, email, phone, country, state, product_interest, message, source, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'new')");
    
    $timestamp = date('Y-m-d H:i:s'); // Use current timestamp
    ...
    if ($stmt->execute()) {
        ...
        // Try to send to Google Sheets as backup/notification
        $sheet_result = sendToGoogleSheets($name, $email, $phone, $country, $state, $product_interest, $message, $timestamp, $source);
        
        // Send email notification
        sendEmailNotification($name, $email, $phone, $country, $state, $product_interest, $message);
        
        http_response_code(200);
        echo "Thank you for your message! We'll get back to you soon. Reference ID: WS-" . str_pad($submission_id, 4, '0', STR_PAD_LEFT);
    } else {
    ...
    echo "Sorry, there was an error saving your message. Please try again or contact us directly at enquiry@mccis.com.my";
